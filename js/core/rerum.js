...
import config, { SHIPPED_GENERATOR } from './config.js'
 ...
 *
 * Deliberately NOT reset by clearStale(): this reports a deployment's
 * configuration, not session state, so a teardown must not re-arm a warning
 * about a config value that has not changed.
 */
...
function requireGenerator() {
    ...
    if (config.GENERATOR === SHIPPED_GENERATOR && generatorWarnedFor !== config.GENERATOR) {
        generatorWarnedFor = config.GENERATOR
        console.warn("DEER is reading and writing as the SHARED sandbox agent that ships as config.GENERATOR's default. Every deployment leaving it unset sees every other one's annotations. Set config.GENERATOR to this deployment's own RERUM agent URI before shipping — and note that this cannot be caught in the sandbox, where every record is generated by that agent, so it stays invisible until production.")
    }
...
function markStale(...ids) {
    ...
    for (const id of ids.flat(3)) {
        ...
        // which expand.targetingClauses deliberately matches and the server
        // merges.  The cache entry that just went stale belongs to the BARE
        // record URI, so the fragment comes off before the RERUM gate --
        // isRerumId refuses anything carrying a hash, which is right for an
        // entity id and wrong for a stale key.
